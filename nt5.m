...
BinScore = sortrows([(1:NBins); sum(out(32:46,:)' == 2) + sum(out(32:46,:)' == 3)*100]',2,'descend')
SelBin = BinScore(1:4,1);
out(47,:) = (out(47,:) < 3) .* (out(47,:)+1) + (out(47,:) > 3) .* (out(47,:));

subplot(3,1,1);
plot(out(1,:), out(SelBin+1,:)*100, "-");
hold on
plot([0 TMax],[1 1]*100,"black--");
plot([0 TMax],[BinThr BinThr]*100,"red-.");
hold off
axis([0 TMax 0 110]);
legend(dec2hex(SelBin(1)), dec2hex(SelBin(2)), dec2hex(SelBin(3)), dec2hex(SelBin(4)), "Overflow", "Alarm");
legend('Location', 'southoutside');
legend('Orientation','horizontal');
xlabel("time [hours]");
ylabel("Bin Level [%]");
title("Bin Level evolution");
...
xlabel("time [hours]");
ylabel("Bin");
title("Garbage Collector State Evolution");

subplot(3,1,3);
plot(out(1,:), out(SelBin+31,:), "-");
legend(dec2hex(SelBin));
legend('Location', 'southoutside');
legend('Orientation','horizontal');
axis([0 TMax 0.9 4.1]);
ylabel("Bin State");
xlabel("time [hours]");
title("Bin State Evolution");
